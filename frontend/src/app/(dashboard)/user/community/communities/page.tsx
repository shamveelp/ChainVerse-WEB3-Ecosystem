...
export default function CommunitiesPage() {
  ...
  const [stats, setStats] = useState<MyCommunitiesStats>({ totalCommunities: 0, adminCommunities: 0, moderatorCommunities: 0, memberCommunities: 0 })
  ...
  const [showLeaveDialog, setShowLeaveDialog] = useState<MyCommunity | null>(null)

  // Load communities
  const loadCommunities = useCallback(async (reset: boolean = false) => {
    ...
    try {
      ...
      const response: MyCommunitiesResponse = await userMyCommunitiesApiService.getMyCommunities(
        activeFilter,
        sortBy,
        cursor,
        20
  ...
  const handleNotificationsToggle = async (e: React.MouseEvent, community: MyCommunity) => {
    e.stopPropagation()
    
    try {
      const newNotificationState = !community.notifications
      await userMyCommunitiesApiService.updateCommunityNotifications(community._id, newNotificationState)
      ...
      toast.success(`Notifications ${newNotificationState ? 'enabled' : 'disabled'} for ${community.communityName}`)
    } catch (error: any) {
  ...
  const handleLeaveCommunity = (e: React.MouseEvent, community: MyCommunity) => {
    e.stopPropagation()
  ...
  const handleLeaveConfirm = async () => {
    ...
    try {
      const result = await userMyCommunitiesApiService.leaveCommunityFromMy(communityToLeave._id)
      
      if (result.success) {
        // Remove from local state
        setCommunities(prev => prev.filter(c => c._id !== communityToLeave._id))
        
        // Update stats
        setStats(prev => ({
          ...prev,
          totalCommunities: Math.max(0, prev.totalCommunities - 1),
          [communityToLeave.memberRole + 'Communities' as keyof MyCommunitiesStats]: 
            Math.max(0, (prev as any)[communityToLeave.memberRole + 'Communities'] - 1)
        }))
    ...
    <div className="flex min-h-screen bg-slate-950">
              ...
              <div className="flex items-center justify-between mb-4">
                <div>
                  ...
                  <p className="text-slate-400">
                    {userMyCommunitiesApiService.formatMemberCount(totalCount)} communities joined
                  </p>
                </div>
              ...
              {stats.totalCommunities > 0 && (
                <div className="grid grid-cols-4 gap-3 mb-4">
                  <div className="text-center">
                    <div className="text-lg font-bold text-white">{stats.totalCommunities}</div>
                    <div className="text-xs text-slate-400">Total</div>
                  </div>
                  <div className="text-center">
                    <div className="text-lg font-bold text-yellow-400">{stats.adminCommunities}</div>
                    <div className="text-xs text-slate-400">Admin</div>
                  </div>
                  <div className="text-center">
                    <div className="text-lg font-bold text-blue-400">{stats.moderatorCommunities}</div>
                    <div className="text-xs text-slate-400">Moderator</div>
                  </div>
                  <div className="text-center">
                    <div className="text-lg font-bold text-green-400">{stats.memberCommunities}</div>
                ...
                <Card className="bg-slate-900/50 backdrop-blur-sm border-slate-700/50 p-12">
                  <div className="text-center space-y-4">
                    ...
                    <p className="text-lg text-slate-400">
                      {searchQuery.trim() ? 'No communities found' : 'No communities yet'}
                    </p>
                        ...
                        <div className="relative">
                          <Avatar className="w-16 h-16 ring-2 ring-slate-700/50 flex-shrink-0">
                            ...
                            <AvatarFallback className="bg-gradient-to-r from-purple-500 to-pink-600 text-white">
                              {userMyCommunitiesApiService.getCommunityAvatarFallback(community.communityName)}
                            </AvatarFallback>
                          </Avatar>
                          {community.unreadPosts > 0 && (
                            <div className="absolute -top-1 -right-1 w-5 h-5 bg-red-500 rounded-full flex items-center justify-center">
                              <span className="text-xs text-white font-bold">
                                {community.unreadPosts > 9 ? '9+' : community.unreadPosts}
                              </span>
                            </div>
                          )}
                        </div>
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-start justify-between mb-2">
                            <div className="min-w-0">
                              ...
                              <p className="text-slate-400 text-sm mb-1">@{community.username}</p>
                            </div>
                            <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                              <Button
                                variant="ghost"
                                size="icon"
                                onClick={(e) => handleNotificationsToggle(e, community)}
                                className="text-slate-400 hover:text-white"
                                title={community.notifications ? 'Disable notifications' : 'Enable notifications'}
                              >
                              ...
                              <Button
                                variant="ghost"
                                size="icon"
                                onClick={(e) => handleSettingsClick(e, community)}
                                className="text-slate-400 hover:text-white"
                              >
                          ...
                          <div className="flex items-center gap-4 mb-3 text-sm text-slate-500">
                            <div className="flex items-center gap-1">
                              ...
                              <span>{userMyCommunitiesApiService.formatMemberCount(community.memberCount)} members</span>
                            </div>
                            ...
                            <span className="text-xs">
                              Joined {userMyCommunitiesApiService.formatTimeAgo(community.joinedAt)}
                            </span>
                          </div>
                          ...
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-4 text-sm text-slate-500">
                              {community.unreadPosts > 0 && (
                                <div className="flex items-center gap-1 text-cyan-400">
                                  ...
                                  <span className="font-medium">{community.unreadPosts} new</span>
                                </div>
                              )}
                              {community.lastActiveAt && (
                                <span>
                                  Active {userMyCommunitiesApiService.formatTimeAgo(community.lastActiveAt)}
                                </span>
                              )}
                            </div>
                            ...
                            <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                              ...
                              >
                                {leavingCommunity === community._id && <Loader2 className="w-4 h-4 mr-1 animate-spin" />}
                                Leave
                              </Button>
                            </div>
                          </div>
      ...
      <Dialog open={!!showLeaveDialog} onOpenChange={() => setShowLeaveDialog(null)}>
        <DialogContent className="sm:max-w-[425px] bg-slate-900 border-slate-700">
          <DialogHeader>
            <DialogTitle className="text-white">Leave {showLeaveDialog?.communityName}?</DialogTitle>
          ...
          <DialogFooter className="gap-2">
            ...
            >
              {leavingCommunity && <Loader2 className="w-4 h-4 mr-2 animate-spin" />}
              Leave Community
            </Button>
          </DialogFooter>
        </DialogContent>
