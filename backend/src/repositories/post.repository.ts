...
import { PostStatsDto } from "../dtos/posts/Post.dto";

@injectable()
export class PostRepository implements IPostRepository {
    
    // Post CRUD operations
    async createPost(authorId: string, content: string, mediaUrls: string[] = [], mediaType: 'none' | 'image' | 'video' = 'none'): Promise<IPost> {
        try {
    ...
    async deletePost(postId: string, authorId: string): Promise<boolean> {
        try {
            if (!Types.ObjectId.isValid(postId) || !Types.ObjectId.isValid(authorId)) {
                throw new CustomError("Invalid post or author ID", StatusCode.BAD_REQUEST);
            }
    ...
    async getFeedPosts(userId: string, cursor?: string, limit: number = 10): Promise<{ posts: IPost[], hasMore: boolean, nextCursor?: string }> {
        try {
            ...
            if (error instanceof CustomError) throw error;
            throw new CustomError("Database error while fetching feed posts", StatusCode.INTERNAL_SERVER_ERROR);
        }
    }

    async getUserPosts(userId: string, viewerUserId?: string, cursor?: string, limit: number = 10): Promise<{ posts: IPost[], hasMore: boolean, nextCursor?: string }> {
        try {
            if (!Types.ObjectId.isValid(userId)) {
                throw new CustomError("Invalid user ID", StatusCode.BAD_REQUEST);
            }
            ...
            if (error instanceof CustomError) throw error;
            throw new CustomError("Database error while fetching user posts", StatusCode.INTERNAL_SERVER_ERROR);
        }
    }

    async getLikedPosts(userId: string, viewerUserId?: string, cursor?: string, limit: number = 10): Promise<{ posts: IPost[], hasMore: boolean, nextCursor?: string }> {
        try {
            if (!Types.ObjectId.isValid(userId)) {
                throw new CustomError("Invalid user ID", StatusCode.BAD_REQUEST);
            }
            ...
            const nextCursor = hasMore && finalLikes.length > 0 
            ...
            if (error instanceof CustomError) throw error;
            throw new CustomError("Database error while fetching liked posts", StatusCode.INTERNAL_SERVER_ERROR);
        }
    }
    ...
    async getTrendingPosts(cursor?: string, limit: number = 10): Promise<{ posts: IPost[], hasMore: boolean, nextCursor?: string }> {
        try {
            ...
            if (error instanceof CustomError) throw error;
            throw new CustomError("Database error while fetching trending posts", StatusCode.INTERNAL_SERVER_ERROR);
        }
    }
    ...
    async getPostsByHashtag(hashtag: string, cursor?: string, limit: number = 10): Promise<{ posts: IPost[], hasMore: boolean, nextCursor?: string }> {
        try {
            ...
            if (error instanceof CustomError) throw error;
            throw new CustomError("Database error while fetching posts by hashtag", StatusCode.INTERNAL_SERVER_ERROR);
        }
    }
    ...
    async searchPosts(query: string, cursor?: string, limit: number = 10): Promise<{ posts: IPost[], hasMore: boolean, nextCursor?: string }> {
        try {
            ...
            if (error instanceof CustomError) throw error;
            throw new CustomError("Database error while searching posts", StatusCode.INTERNAL_SERVER_ERROR);
        }
    }
    ...
    async likePost(userId: string, postId: string): Promise<ILike> {
        try {
            if (!Types.ObjectId.isValid(userId) || !Types.ObjectId.isValid(postId)) {
                throw new CustomError("Invalid user or post ID", StatusCode.BAD_REQUEST);
            }
    ...
    async getPostLikes(postId: string, cursor?: string, limit: number = 20): Promise<{ likes: ILike[], hasMore: boolean, nextCursor?: string }> {
        try {
            ...
            if (error instanceof CustomError) throw error;
            throw new CustomError("Database error while fetching post likes", StatusCode.INTERNAL_SERVER_ERROR);
        }
    }

    // Comment operations
    async createComment(authorId: string, postId: string, content: string, parentCommentId?: string): Promise<IComment> {
        try {
            if (!Types.ObjectId.isValid(authorId) || !Types.ObjectId.isValid(postId)) {
                throw new CustomError("Invalid author or post ID", StatusCode.BAD_REQUEST);
            }
    ...
    async updateComment(commentId: string, content: string): Promise<IComment | null> {
        try {
            ...
            if (error instanceof CustomError) throw error;
            throw new CustomError("Database error while updating comment", StatusCode.INTERNAL_SERVER_ERROR);
        }
    }
    ...
    async deleteComment(commentId: string, authorId: string): Promise<boolean> {
        try {
            if (!Types.ObjectId.isValid(commentId) || !Types.ObjectId.isValid(authorId)) {
                throw new CustomError("Invalid comment or author ID", StatusCode.BAD_REQUEST);
            }
            ...
            if (comment) {
                // Update counts
                await this.updatePostCounts(comment.post.toString(), 'commentsCount', -1);
                
                if (comment.parentComment) {
                    await this.updateCommentCounts(comment.parentComment.toString(), 'repliesCount', -1);
                }
            ...
            if (error instanceof CustomError) throw error;
            throw new CustomError("Database error while deleting comment", StatusCode.INTERNAL_SERVER_ERROR);
        }
    }

    async getPostComments(postId: string, cursor?: string, limit: number = 10, parentCommentId?: string): Promise<{ comments: IComment[], hasMore: boolean, nextCursor?: string }> {
        try {
            if (!Types.ObjectId.isValid(postId)) {
                throw new CustomError("Invalid post ID", StatusCode.BAD_REQUEST);
            }
            ...
            if (error instanceof CustomError) throw error;
            throw new CustomError("Database error while fetching post comments", StatusCode.INTERNAL_SERVER_ERROR);
        }
    }
    ...
    async getCommentReplies(commentId: string, cursor?: string, limit: number = 10): Promise<{ comments: IComment[], hasMore: boolean, nextCursor?: string }> {
        try {
            ...
            if (error instanceof CustomError) throw error;
            throw new CustomError("Database error while fetching comment replies", StatusCode.INTERNAL_SERVER_ERROR);
        }
    }
    ...
    async likeComment(userId: string, commentId: string): Promise<ICommentLike> {
        try {
            if (!Types.ObjectId.isValid(userId) || !Types.ObjectId.isValid(commentId)) {
                throw new CustomError("Invalid user or comment ID", StatusCode.BAD_REQUEST);
            }
    ...
    async updateCommentCounts(commentId: string, field: 'likesCount' | 'repliesCount', increment: number): Promise<void> {
        try {
            if (!Types.ObjectId.isValid(commentId)) {
                throw new CustomError("Invalid comment ID", StatusCode.BAD_REQUEST);
            }
    ...
    async getPostStats(userId?: string): Promise<PostStatsDto> {
        try {
            const matchQuery = userId && Types.ObjectId.isValid(userId) 
            ...
            throw new CustomError("Database error while fetching post stats", StatusCode.INTERNAL_SERVER_ERROR);
        }
    }
    ...
    async getPopularHashtags(limit: number = 10): Promise<string[]> {
        try {
            ...
            throw new CustomError("Database error while fetching popular hashtags", StatusCode.INTERNAL_SERVER_ERROR);
        }
    }

    // Admin operations
    async findPostsByUser(userId: string, includeDeleted: boolean = false): Promise<IPost[]> {
        try {
            if (!Types.ObjectId.isValid(userId)) {
                throw new CustomError("Invalid user ID", StatusCode.BAD_REQUEST);
            }
    ...
    async bulkDeletePosts(postIds: string[]): Promise<boolean> {
        try {
            ...
            throw new CustomError("Database error while bulk deleting posts", StatusCode.INTERNAL_SERVER_ERROR);
        }
    }
