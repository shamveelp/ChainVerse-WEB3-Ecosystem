...
export interface IUserQuestRepository {
  ...
  getQuestLeaderboard(questId: string, page: number, limit: number): Promise<{ participants: any[]; total: number; pages: number }>;
  ...
  // Validation
  validateTaskSubmission(taskId: string, submissionData: any): Promise<{ valid: boolean; message?: string }>;
